...
add_filter( 'go_stats_endpoint', function( $stats ) {
    ...
    $courses_completed = $wpdb->get_var( "SELECT COUNT(*) FROM wp_learndash_user_activity WHERE WHERE `activity_completed` > 0" );

    $users_course_completed = $wpdb->get_var( "SELECT COUNT(DISTINCT(user_id)) FROM wp_learndash_user_activity WHERE WHERE `activity_completed` > 0" );
