jQuery(document).ready(function($) {
    // Zume historical stats form handler
    $('#zume-historical-stats-form').on('submit', function(e) {
        e.preventDefault();
        
        const startDate = $('#zume_start_date').val();
        const endDate = $('#zume_end_date').val();
        ...
        // Show loading state
        $('#run-zume-stats').prop('disabled', true);
        $('#zume-loading').show();
        $('#zume-results').hide();
        $('#zume-error').hide();
        ...
        // Make AJAX request
        $.ajax({
            url: zumeHistoricalStats.ajaxurl,
            type: 'POST',
            data: {
                action: 'run_zume_historical_stats',
                start_date: startDate,
                end_date: endDate,
                _ajax_nonce: zumeHistoricalStats.nonce
            },
            ...
            complete: function() {
                $('#run-zume-stats').prop('disabled', false);
                $('#zume-loading').hide();
            }
        });
    });
    ...
    function showResults(data) {
        ...
        if (data.results && data.results.length > 0) {
            ...
            data.results.forEach(function(result) {
                ...
                if (result.status === 'success' && result.metrics) {
                    details = `Registered Users: ${result.metrics.registered_users}`;
        ...
        $('#zume-results-content').html(html);
        $('#zume-results').show();
    }
    
    function showError(message) {
        $('#zume-error-content').text(message);
        $('#zume-error').show();
        $('#zume-results').hide();
    }

    // Save API key form handler
    $('#zume-api-key-form').on('submit', function(e) {
        e.preventDefault();
        const apiKey = $('#zume_api_key').val();
        if (!apiKey) {
            $('#zume-api-key-message').html('<span style="color:red;">Please enter an API key.</span>');
            return;
        }

        $('#zume-save-api-key').prop('disabled', true);
        $('#zume-api-key-loading').show();
        $('#zume-api-key-message').empty();

        $.ajax({
            url: zumeHistoricalStats.ajaxurl,
            type: 'POST',
            data: {
                action: 'save_zume_stats_key',
                api_key: apiKey,
                _ajax_nonce: zumeHistoricalStats.nonce
            },
            success: function(response) {
                ...
                if (data && data.success) {
                    ...
                    $('#zume-api-key-message').html('<span style="color:green;">API key saved.</span>');
                    if ($('#zume-api-key-status').length) {
                        $('#zume-api-key-status').html('<strong>✅ API key configured:</strong> ' + preview).css('color', 'green');
                    } else {
                        $('<p id="zume-api-key-status" style="color: green;"><strong>✅ API key configured:</strong> ' + preview + '</p>').insertBefore('#zume-api-key-form');
                    }
                    $('#zume_api_key').val('');
                } else {
                    const msg = data && data.data && data.data.message ? data.data.message : 'Failed to save API key';
                    $('#zume-api-key-message').html('<span style="color:red;">' + msg + '</span>');
                }
            },
            error: function() {
                $('#zume-api-key-message').html('<span style="color:red;">Request failed.</span>');
            },
            complete: function() {
                $('#zume-save-api-key').prop('disabled', false);
                $('#zume-api-key-loading').hide();
            }
        });
    });
});